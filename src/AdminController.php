...
class AdminController {
  ...
  public function publishVersion(int $templateId, string $title, string $description, string $fieldsJson, int $userId, ?string $contentHtml=null, ?string $printCss=null): int {
    ...
    // A published version MUST set is_published=1 + published_at, otherwise
    // has_published_version(templateId) stays false and create_waiver returns
    // no_published_version (the whole point of "publish"). The columns default
    // to is_published=0 / published_at NULL (migrations/001_init.sql), so an
    // INSERT that omits them silently produces an unpublished row.
    $stmt=$this->db->pdo()->prepare('INSERT INTO waiver_template_versions (template_id, version, title, description, fields_json, requires_signature, created_by, created_at, content_html, print_css, is_published, published_at) VALUES (?,?,?,?,?,1,?,UTC_TIMESTAMP(),?,?,1,UTC_TIMESTAMP())');
